<?php

return [
    'throttle' => 'Слишком много попыток. Пожалуйста, попробуйте еще раз через :seconds секунд(ы)',
    'menu' => [
        'prizes' => 'Призы',
        'weekly_prizes' => 'Еженедельные призы',
        'instant_prizes' => 'Моментальные призы',
        'vouchers' => 'Ваучеры',
        'prize_drawing_calendars' => 'Календарь розыгрыша',
        'receipt_statuses' => 'Статусы чеков',
        'receipts' => 'Чеки',
        'settings' => 'Настройки',
        'users' => 'Участники',
        'games' => 'Игры',
        'invitations' => 'Приглашение',
        'charts' => 'Статистики',
        'registration' => 'Пользователи'
    ],
    'fields' => [
        'name_ru' => 'Наименование на русском',
        'name_kk' => 'Наименование на казахском',
        'name_uz' => 'Наименование на узбекском',
        'bal' => 'Балл',
        'number' => 'Количество',
        'code' => 'Код',
        'count' => 'Количество',
        'codes' => 'Коды',
        'winning_date' => 'Дата и время выигрыша',
        'prize' => 'Приз',
        'prizes' => 'Призы',
        'user' => 'Пользователь',
        'spent_balls' => 'Потраченные баллы',
        'is_approved' => 'Подтверждение выигрыша',
        'drawing_at' => 'Дата и время проведения',
        'started_at' => 'Начало',
        'is_finished' => 'Проведён?',
        'start_date' => 'Дата и время начало',
        'repeat_weeks' => 'Сколько недель?',
        'color' => 'Цвет',
        'image' => 'Фото',
        'created_at' => 'Дата создания',
        'status' => 'Статус',
        'game_max_coins' => 'Количество коинов за прохождение одной игры',
        'receipt_life' => 'Количество жизней при загрузке чека',
        'referal_life' => 'Количество жизней для рефералки',
        'promotion_date' => 'Дата начало и окончания акции',
        'winner' => 'Победитель',
        'receipt_id' => 'Номер чека',
        'name' => 'ФИО',
        'phone_number' => 'Номер телефона',
        'email' => 'E-mail',
        'life' => 'Жизней',
        'level' => 'Уровень',
        'coin' => 'Коины',
        'games' => 'Игры',
        'before_life' => 'Жизнь до',
        'after_life' => 'Жизнь после',
        'before_coins' => 'Коины до',
        'coins' => 'Коин',
        'after_coins' => 'Коины после',
        'before_level' => 'Уровень до',
        'after_level' => 'Уровень после',
        'score' => 'Очки',
        'time' => 'Продолжительность в сек',
        'is_finished' => 'Завершен?',
        'default_life' => 'Количество жизней при регистрации',
        'friend' => 'Друг',
        'invitations' => 'Приглашение',
        'receipts' => 'Чеки',
        'instant_prizes' => 'Моментальные призы',
        'vouchers' => 'Ваучеры',
        '' => '',
    ],
    'hints' => [
        'bal' => 'Стоимость в баллах',
        'number' => 'Общее количество приза',
    ],
    'buttons' => [
        'showbox_add' => 'Добавить Шоубокс',
        'balance_add' => 'Добавить Баланс',
        'add' => 'Добавить',
        'winned' => 'Выигранные',
        'free' => 'Свободные',
        'import' => 'Импорт',
        'export' => 'Экспорт',
        'approve' => 'Подтвердить',
        'reject' => 'Отклонить',
        'to_approve' => 'Ждет подтверждение'
    ],
    'messages' => [
        'added' => 'Добавлено',
        'added_with_count' => 'Добавлено :count записей',
        'signUp_error' => 'Не удалось зарегистрироваться! Возможно Вы уже успешно прошли регистрацию или попробуйте другую почту.',
        'sms_message' => 'Ваш пароль на сайте orbit-promo.kz :code',
        'verify_error' => 'Неправильный код!',
        'auth_failed' => 'Неверный номер телефона или код!',
        'sms_limit' => 'Слишком много попыток. Пожалуйста, попробуйте позже!',
        'email_limit' => 'Слишком много попыток. Пожалуйста, попробуйте позже!',
        'finish_registration' => 'Вы не завершили регистрацию!',
        'saved' => 'Сохранено!',
        'life_error' => 'У вас нет жизней!',
        'error_finished_game' => 'Ошибка при завершение игры',
        'qr_not_found' => 'Qr код не найден!',
        'orbit_not_found' => 'Орбит не найдено в чеке!',
        'not_enough_coins' => 'Не хватает коинов!',
        'approve_body' => 'Вы действительно хотите подтвердить наличие позиции "Орбит"?',
        'reject_body' => 'Вы действительно хотите отклонить наличие позиции "Орбит"?',
        'error' => 'Ошибка',
        'duplicate' => 'Дубликат',
        'unique_users' => 'Уникальных участников промо',
        'games_played' => 'Всего игр сыграно',
        'unique_gamers' => 'Всего уникальных игроков',
        'average_game_count' => 'Среднее число игр на игрока',
        'average_game_users_count' => 'Среднее число игр на участника промо',
        'total_returning_players' => 'Всего возвращающихся игроков (которые возвращаются на следующий день)',
        'games_first_level' => 'Игр на первом уровне',
        'unique_players_first_level' => 'Уникальных игроков на первом уровне:',
        'games_other_level' => 'Игр на следующих уровнях',
        'unique_gamers_next_level' => 'Уникальных игроков на следующих уровнях',
        'avg_game_players_next_level' => 'Среднее число игр на игрока на следующих уровнях',
        'scan_count' => 'Всего чеков',
        'accepted_count' => 'принято',
        'not_accepted_count' => 'не принято ',
        'unique_scanned_users_count' => 'Уникальных сканирующих',
        'avg_scanned_users_count' => 'Среднее число сканов на участника промо',
        'users' => 'Пользователи',
        'registration_users' => 'Регистрации на сайте',
        'not_verified_users' => 'Не верифицировавших номер',
        '' => '',
        '' => '',
        '' => '',
        '' => '',
    ],
    'mail' => [
        'subject' => 'Ваш пароль для Orbit promo',
        'title' => 'Здравствуйте, :fullName !',
        'code' => 'Ваш пароль для входа в систему: :code'
    ]
];
